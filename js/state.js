...
window.MazeApp.state = {
  ...
  // ----- timer -----
  // elapsedMs is an ACCUMULATOR (not a raw timestamp diff) so its rate can
  // be temporarily slowed by a heart pickup — see js/game.js's tick().
  elapsedMs: 0,
  hasMoved: false,
  timerInterval: null,
  parSteps: 0,                // shortest-path length, used for star thresholds

  // ----- collectibles (Forest-style worlds only; see js/collectibles.js) -----
  collectibles: {},           // "x,y" -> {type: 'star'|'flash'|'heart'|'greenHeart'}
  doors: {},                  // "x,y" -> {locked: bool, cost: number}
  shards: 0,                  // star shards currently held THIS level
  starEverCollected: false,   // true once any star has been picked up this level (reveals green heart)
  slowUntil: 0,                // performance.now() timestamp; timer runs slow until then
  slowFactor: 1,                // timer rate multiplier while a heart buff is active (<1 = slower)
  flashBoostUntil: 0,          // performance.now() timestamp; torch radius boosted until then
  flashBoostAmount: 0,         // extra fog radius while boosted
