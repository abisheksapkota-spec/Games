...
(function(){
  ...
  /**
   * Breadth-first search returning the actual shortest path (list of
   * {x,y} cells from start to goal inclusive), or null if unreachable.
   * Used by collectibles.js to know which cells are "on the required
   * route" vs. optional side branches.
   */
  function shortestPath(grid, width, height, sx, sy, gx, gy){
  ...
  /**
   * Breadth-first search for the shortest path LENGTH only (a thin
   * wrapper around shortestPath, kept for callers that just need the
   * number, e.g. star-rating par time).
   */
  function shortestPathLen(grid, width, height, sx, sy, gx, gy){
    const path = shortestPath(grid, width, height, sx, sy, gx, gy);
    return path ? path.length - 1 : -1;
  }

  window.MazeApp.MazeGenerator = { mulberry32, shuffle, generateMaze, shortestPath, shortestPathLen };
})();
